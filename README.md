...
To inspect Google indexing, verify the URL-prefix property `https://yy3462-create.github.io/` in Google Search Console using its supplied HTML verification tag or file, then submit `sitemap.xml` and request indexing for the homepage and `svd.html`. Google account sign-in is required; no verification token has been invented or added.
