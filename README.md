...
The homepage has a canonical URL, a name-and-affiliation title, and ProfilePage/Person JSON-LD identifying Yue Yao and her existing GitHub and LinkedIn profiles. Open Graph metadata supplies shared-link previews. `robots.txt` allows crawling and points to `sitemap.xml`, whose last-modified dates reflect actual page changes. These make the site understandable and discoverable; they do not guarantee Google indexing or name-query ranking.

To inspect Google indexing, verify the URL-prefix property `https://yy3462-create.github.io/` in Google Search Console using its supplied HTML verification tag or file, then submit `sitemap.xml` and request indexing for the homepage and `svd.html`. Google account sign-in is required. The Google-supplied HTML verification tag is retained in the homepage; preserve it when editing the page to maintain ownership verification.
